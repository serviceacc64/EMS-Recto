    ...
    <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 mb-8">
      <div className="flex items-center gap-4">
        <div className="relative">
          <div className="w-14 h-14 rounded-full bg-accent flex items-center justify-center text-accent-text text-xl font-bold shadow-lg shadow-accent/20">
            <i className="fas fa-user-shield"></i>
          </div>
          <div className="absolute -bottom-1 -right-1 w-5 h-5 bg-green-500 border-4 border-base rounded-full"></div>
        </div>
        <div>
          <p className="text-text-muted text-sm font-semibold m-0">{pageInfo.subtitle}</p>
          <h1 className="text-text-main text-2xl md:text-3xl font-extrabold tracking-tight m-0">{pageInfo.title}</h1>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 w-full lg:w-auto relative">
        ...
        <div className="relative flex-1 lg:w-72">
          ...
          <input 
            ...
            className="w-full bg-surface border border-border-subtle rounded-2xl py-3 pl-11 pr-4 text-sm font-medium focus:outline-none focus:ring-4 focus:ring-accent/10 focus:border-accent transition-all shadow-sm"
          />
        ...
        {/* Notifications */}
        <div className="relative">
          <button onClick={() => setShowNotifications(!showNotifications)} className="w-12 h-12 bg-surface border border-border-subtle rounded-2xl flex items-center justify-center text-text-main hover:bg-surface-alt transition-colors shadow-sm relative group">
            <i className={`fas fa-bell ${notifications.length > 0 ? 'animate-bounce' : ''}`}></i>
            {notifications.length > 0 && (
              <span className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white text-[10px] font-bold flex items-center justify-center rounded-full border-2 border-surface animate-pulse">
          ...
          {showNotifications && (
            <>
              ...
              <div className="absolute right-0 top-14 w-80 bg-surface border border-border-subtle rounded-[24px] shadow-xl z-30 overflow-hidden animate-[slideIn_0.2s_ease-out]">
                <div className="p-4 border-b border-border-subtle bg-surface-alt">
                  <h4 className="text-text-main font-bold text-sm m-0">Notifications</h4>
                </div>
                <div className="max-h-96 overflow-y-auto">
                  {notifications.length === 0 ? (
                    <div className="p-8 text-center text-text-muted text-xs font-medium">All caught up!</div>
                  ) : (
                    notifications.map((n, i) => (
                      <div key={i} onClick={() => { setSelectedNotification(n); setShowNotifications(false); }} className="p-4 border-b border-border-subtle last:border-0 hover:bg-surface-alt transition-colors cursor-pointer group">
                        <div className="flex gap-3">
                          <div className={`w-8 h-8 rounded-lg ${n.color.replace('text', 'bg')}/10 ${n.color} flex items-center justify-center text-xs shrink-0`}><i className={`fas ${n.icon}`}></i></div>
                          <div>
                            <p className="text-text-main text-[13px] font-bold m-0 group-hover:text-accent transition-colors">{n.title}</p>
                            <p className="text-text-muted text-[11px] font-medium mt-0.5 leading-relaxed m-0">{n.message}</p>
                          </div>
                        </div>
                      </div>
                    ))
